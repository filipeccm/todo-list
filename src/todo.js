...
import { Project, addToProject, projects } from "./project"
import displayProjs from "./ui-project";
...
const addTodo = () => {
  ...
  const newTodo = new Todo(form.elements['title'].value, 
...
const toggleTodo = (event) => {
  const projParent = event.target.parentNode.parentNode.parentNode.parentNode.id;
...
const editDate = (event) => {
  ...
  dateChange.addEventListener('change', function(event) {
    ...
    for(let i = 0; i < projList.length; i++) {
    if(projList[i].name === projParent) {
      console.log(newCalendar.altInput.value)
...
export { Todo, addTodo, deleteTodo, toggleTodo, setTodosIndex, editTodo, saveEdit, editPriority, editDate}